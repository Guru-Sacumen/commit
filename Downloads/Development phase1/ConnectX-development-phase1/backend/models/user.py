...
from sqlalchemy import Column, String, DateTime, ForeignKey, Boolean, Enum, Index, Integer
...
class PasswordReset(Base):
    """
    Password reset token model with 6-digit verification code support.
    
    Attributes:
        token: Unique reset token (primary key)
        user_id: Foreign key to user
        verification_code: 6-digit verification code for email verification
        expires_at: Code expiration timestamp (10 minutes)
        attempts: Number of verification attempts (max 5)
        is_verified: Whether the code has been verified
        created_at: Timestamp of creation
    """

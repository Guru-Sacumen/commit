...
def _ensure_runtime_schema() -> None:
    ...
    try:
        with engine.begin() as conn:
            ...
            if "connector_catalog" in table_names:
                existing_cols = {
                    column["name"] for column in inspector.get_columns("connector_catalog")
                }
                for col_name, col_def in connector_catalog_additions.items():
                    if col_name in existing_cols:
                        continue
                    conn.execute(
                        text(
                            f"ALTER TABLE connector_catalog "
                            f"ADD COLUMN {col_name} {col_def}"
                        )
                    )
            if "connectors" in table_names:
                ...
                for col_name, col_def in connector_additions.items():
                    if col_name in existing_cols:
                        continue
                    conn.execute(
                        text(
                            f"ALTER TABLE connectors "
                            f"ADD COLUMN {col_name} {col_def}"
                        )
                    )
        ...
        logger.warning(f"[schema] runtime schema update skipped: {exc}")
    ...
    try:
        with engine.begin() as conn:
            ...
            try:
                ...
                logger.info("[constraint] Dropped uq_tenant_single_admin constraint to allow multiple admins")
            except Exception as e:
                logger.debug(f"[constraint] Could not drop uq_tenant_single_admin (may not exist): {e}")
                
            # Note: We're no longer enforcing single admin per tenant
            # Multiple admins are now allowed per tenant
            
    except Exception as exc:
        logger.warning(f"[constraint] Warning: Could not modify admin constraints: {exc}")
...
def _load_connector_catalog_from_excel() -> list[dict]:
    ...
    if not workbook_path:
        logger.warning(f"[catalog] workbook not found: {workbook_name}")
        return []

    try:
        import openpyxl  # type: ignore
    except Exception as exc:
        logger.warning(f"[catalog] openpyxl unavailable: {exc}")
        return []
...
def _sync_connector_catalog_from_excel() -> None:
    ...
    try:
        ...
        if not rows:
            logger.info("[catalog] no rows loaded from excel")
            return
        ...
        logger.info(f"[catalog] {action} {len(rows)} connector catalog rows")
    except Exception as exc:
        db.rollback()
        logger.error(f"[catalog] failed seeding connector catalog: {exc}")
    finally:
        db.close()
...
FROM_ADDR = os.getenv("FROM_ADDR", "no-reply@connectx.com")
...
cors_origins_env = os.getenv("CORS_ORIGINS", "http://localhost:3000,http://127.0.0.1:3000,http://localhost:3001,http://127.0.0.1:3001,http://localhost:5173,http://127.0.0.1:5173")
...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    logger.info(f"User {user.email} logged in, tenant_id={tenant_id}, role={role}")
...
@app.post("/api/v1/auth/mfa/enable", response_model=dict)
def enable_mfa(
...
@app.post("/api/v1/auth/mfa/disable", response_model=dict)
def disable_mfa(
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
@app.post("/api/v1/auth/totp/setup", response_model=TOTPSetup)
def setup_totp(
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
@app.post("/api/v1/auth/totp/verify", response_model=dict)
def verify_totp(
    payload: TOTPVerify,
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
...
@app.post("/api/v1/auth/totp/disable", response_model=dict)
def disable_totp(
    user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
@app.post("/api/v1/auth/login-with-totp", response_model=Token)
...
def forgot_password(req: ForgotPasswordRequest, db: Session = Depends(get_db)):
    ...
    if SMTP_HOST:
        ...
        return {"ok": True, "message": "Password reset link generated", "link": link}

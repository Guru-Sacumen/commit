...
def create_super_admin() -> Optional[User]:
    ...
    If a super admin already exists, returns the existing user.
    Otherwise, creates a new super admin with credentials from environment variables.
    
    Returns:
        Optional[User]: The existing or created super admin user, or None if error occurs.
    ...
    try:
        ...
        logger.info(f"Checking for existing super admins...")
        ...
        if existing_superadmin:
            ...
            logger.info(f"Name: {existing_superadmin.full_name}")
            logger.info(f"ID: {existing_superadmin.id}")
            return existing_superadmin
        
        # Create new super admin user
        logger.info(f"No existing super admin found. Creating new super admin: {email}")
        superadmin = User(
            id=str(uuid.uuid4()),
            email=email,
            full_name=full_name,
        ...
        logger.info(f"Successfully created super admin: {email}")
        logger.info(f"Name: {full_name}")
        logger.info(f"ID: {superadmin.id}")
        logger.info("Password set from SUPERADMIN_PASSWORD env variable")
...
def create_sample_tenant() -> Optional[Tenant]:
    ...
    - One admin user (demo-admin@connectx.dev)
    - One member user (demo-user@connectx.dev)
    ...
    try:
        ...
        admin_email = "demo-admin@connectx.dev"
        ...
        member_email = "demo-user@connectx.dev"

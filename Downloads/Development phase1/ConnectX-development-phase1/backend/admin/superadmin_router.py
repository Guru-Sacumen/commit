...
from auth import create_user_with_membership, get_current_superadmin, get_db, generate_secure_password
...
def create_company(
    ...
    try:
        ...
        email_sent = email_service.send_admin_invite_email(
        ...
        if not email_sent:
            ...
            print(f"Warning: Failed to send admin invite email to {payload.admin_email}")
        else:
            print(f"Success: Admin invite email sent to {payload.admin_email}")
    ...
    response_data = {"tenant_id": tenant.id, "admin_id": user.id}

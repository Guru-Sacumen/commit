...
function UserManagement({ 
  ...
  const [resendInvitationConfirm, setResendInvitationConfirm] = useState({ open: false, userId: null, userName: '' });
  ...
  // Auto-generate password when opening create form
  const handleAddUser = () => {
    setEditingUser(null);
    setForm({
      email: '',
  ...
  const openResendInvitationConfirm = (userId, userName) => {
    setResendInvitationConfirm({ open: true, userId, userName });
  };

  const closeResendInvitationConfirm = () => {
    setResendInvitationConfirm({ open: false, userId: null, userName: '' });
  };
  ...
  const resetForm = () => {
    setForm({ 
      email: '', 
      fullName: '', 
      password: generatePassword(), // Generate new password for next use
      role: 'MEMBER' 
    });
    setEditingUser(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingUser) {
        ...
        // First update user details (if any)
        ...
        if (form.password) {
          const passwordRes = await fetch(`${API_BASE_URL}/admin/${currentTenant}/users/${editingUser.id}/password`, {
            method: 'PATCH',
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
          ...
          if (!passwordRes.ok) {
            ...
            throw new Error(extractErrorMessage(errorText, 'Failed to update password'));
          }
        }
        ...
        const displayPassword = created.generated_password || form.password;
        setSuccessMessage(
          `User created successfully! Password: ${displayPassword}`
        );
  ...
  const confirmResendInvitation = async () => {
    const { userId } = resendInvitationConfirm;
    closeResendInvitationConfirm();

    try {
      const res = await fetch(`${API_BASE_URL}/admin/${currentTenant}/users/${userId}/resend-invitation`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });

      if (!res.ok) {
        throw new Error(extractErrorMessage(await res.text(), 'Failed to resend invitation'));
      }

      const result = await res.json();
      showSnackbar(result.message || 'Invitation email sent successfully');
      ...
      console.error(err);
      showSnackbar(err.message || 'Unable to resend invitation', 'error');
    }
  };
  ...
  return (
    ...
    >
      <DialogTitle>
        {editingUser ? `Editing ${editingUser.full_name || editingUser.email}` : 'Create New User'}
        <IconButton
          aria-label="close"
          onClick={resetForm}
          sx={{
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
            ...
            <Box>
              <TextField
                ...
                placeholder={editingUser ? "Enter new password to change" : "Auto-generated password"}
                value={form.password}
                ...
                helperText={
                  !editingUser 
                    ? "Password is automatically generated and will be sent via email" 
              ...
              {editingUser && (
                <Chip 
                  label="Optional: Enter new password to update" 
                  size="small" 
                  color="default" 
                  variant="outlined"
                  sx={{ mt: 1, fontSize: '0.75rem' }}
              ...
              <CardContent sx={{ p: 3, '&:last-child': { pb: 3 } }}>
                ...
                <Box sx={{ 
                  ...
                  <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                    {/* Reinvite Email button */}
                    <Button 
                      size="small" 
                      variant="outlined" 
                      onClick={() => openResendInvitationConfirm(user.id, user.email)}
                      sx={{ fontSize: '0.75rem', py: 0.5 }}
                      startIcon={<MailIcon sx={{ fontSize: '0.875rem' }} />}
                    >
                      Reinvite
                    </Button>
                    {/* Reset Google Authenticator button */}
                    <Button 
                      size="small" 
                      variant="outlined" 
    ...
    <Dialog
      open={resendInvitationConfirm.open}
      onClose={closeResendInvitationConfirm}
      maxWidth="sm"
      fullWidth
    >
      <DialogTitle>
        ...
        <IconButton
          aria-label="close"
          onClick={closeResendInvitationConfirm}
          sx={{
            position: 'absolute',
            right: 8,
            top: 8,
      ...
      <DialogContent>
        <Typography sx={{ mb: 2 }}>
          Are you sure you want to resend the invitation email to <strong>{resendInvitationConfirm.userName}</strong>?
        </Typography>
        <Typography variant="body2" color="text.secondary">
      ...
      <DialogActions>
        <Button onClick={closeResendInvitationConfirm} color="primary">
          Cancel
        </Button>
        <Button onClick={confirmResendInvitation} color="success" variant="contained" startIcon={<MailIcon />}>
          Resend Invitation
        </Button>
      </DialogActions>
    </Dialog>

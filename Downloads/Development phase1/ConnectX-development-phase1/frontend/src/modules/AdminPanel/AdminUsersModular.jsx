...
export default function AdminUsersModular({ showOnlyCompanies = false }) {
  ...
  const [pendingBulkRejectRequests, setPendingBulkRejectRequests] = useState([]);
  ...
  async function processConnectorRequestDecision(request, action, reason = '') {
    ...
    try {
      ...
      if (mappedAction === 'decline' && reason.trim()) {
  ...
  async function handleConnectorRequestDecision(request, action) {
    ...
    if (mappedAction === 'decline') {
      ...
      setPendingBulkRejectRequests([]);
      setRejectReason('');
      setRejectDialogOpen(true);
      return;
    }
    await processConnectorRequestDecision(request, action);
  }
  ...
  async function handleBulkConnectorRequestDecision(action) {
    ...
    if (mappedAction === 'decline') {
      ...
      setPendingBulkRejectRequests(pendingSelection);
      setRejectReason('');
      setRejectDialogOpen(true);
      return;
    }
  ...
  async function submitRejectReason() {
    ...
    const bulkRequests = pendingBulkRejectRequests;
    const singleRequest = pendingRejectRequest;
    ...
    if (bulkRequests.length > 0) {
      for (const request of bulkRequests) {
        // eslint-disable-next-line no-await-in-loop
        await processConnectorRequestDecision(request, 'decline', reason);
      }
      setSelectedRequestIds(new Set());
      return;
    }
    if (singleRequest) {
      await processConnectorRequestDecision(singleRequest, 'decline', reason);
    }
  }
      ...
      try {
        ...
        connectorsData.forEach((connector) => {
          ...
          purchasedMeta.set(rawId, {
            ...
            status: connector.status || 'IN_PROGRESS',
            version_name: connector.version_name || 'v1.0.0',
          });
        });
    ...
    >
      ...
      {!showOnlyCompanies && !isSuper && adminCompany && (
        <TenantDetailView
          ...
          onConnectorRequestDecision={handleConnectorRequestDecision}
          ...
          onBulkConnectorRequestDecision={handleBulkConnectorRequestDecision}
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            {pendingBulkRejectRequests.length > 0
              ? `Provide a rejection reason for ${pendingBulkRejectRequests.length} selected request(s).`
              : 'Provide a rejection reason for this connector request.'}
          </Typography>
          <TextField
            autoFocus
            fullWidth
        ...
        <DialogActions>
          ...
          <Button variant="contained" color="error" onClick={submitRejectReason}>
            Submit Rejection
          </Button>
        </DialogActions>
      </Dialog>

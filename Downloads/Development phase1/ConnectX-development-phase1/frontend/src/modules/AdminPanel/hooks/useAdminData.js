...
export function useAdminData() {
      ...
      Array.from(purchasedConnectorIds).map((connectorId) => {
        ...
        if (fromCatalog) {
          return {
            ...
            version_name: fallback?.version_name || fromCatalog.version_name || 'v1.0.0',
          };
        }
        return {
          id: connectorId,
          ...
          version_name: fallback?.version_name || 'v1.0.0',
        };
      }),
  ...
  async function fetchConnectorCatalog() {
    let rows = [];
    const superadminRes = await fetch(`${API_BASE_URL}/integration/superadmin/catalog`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (superadminRes.ok) {
      rows = await superadminRes.json();
    } else {
      const legacyRes = await fetch(`${API_BASE_URL}/connectors/catalog`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!legacyRes.ok) {
        throw new Error(extractErrorMessage(await legacyRes.text(), 'Failed to load connector catalog'));
      }

...
export default function LoginPage() {
  ...
  // Get the redirect destination from AuthGuard (if user was redirected from a protected route)
  ...
  const handleSuccessfulLogin = async (tokenData) => {
    ...
    // If user was redirected from a protected route (e.g., email link), go back there
    if (from) {
      console.log('LoginPage - Redirecting to original destination:', from);
  ...
  const handleTOTPVerify = async (totpCode) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/auth/login-with-totp`, {
        method: 'POST',
        headers: {
